...
class adapter(persistence.port):
    ...
    def login(self):
        ...
        ddd = { "UserName":"manager", "Password":"Orakom2024", "CompanyDB":"SBO_ORAKOM99",}
    ...
    async def query(self, *services, **constants):
        ...
        url = f"{self.config['url']}/{constants['path']}"
    ...
    async def read(self, storekeeper, **constants):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, headers=head,ssl=False) as response:
